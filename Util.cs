            ...
            using(WebClient client = new WebClient()) {
                for (int i = 0; i < employees.Count; i++) {
                    ...
                    graphic.DrawString(
                        ...
                        new Rectangle(COMPANY_NAME_START_X, COMPANY_NAME_START_Y, BADGE_WIDTH, COMPANY_NAME_WIDTH
                        ), format
                    );
                    ...
                    graphic.DrawString(
                        ...
                        brush,
                        new Rectangle(
                            EMPLOYEE_NAME_START_X, EMPLOYEE_NAME_START_Y, BADGE_WIDTH, EMPLOYEE_NAME_HEIGHT
                        ), format
                    );
                    ...
                    graphic.DrawString(
                        ...
                        brush,
                        new Rectangle(
                            EMPLOYEE_ID_START_X, EMPLOYEE_ID_START_Y, EMPLOYEE_ID_WIDTH, EMPLOYEE_ID_HEIGHT
                        ), format
                    );
                    string template = "data/{0}_employeeBadge.png";
                    badge.Save(String.Format(template, employees[i].GetId()));
                }
            }
            Console.WriteLine("Badges created!");            
        }
    }
}